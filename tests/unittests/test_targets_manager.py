...
MODULE_PATH = "browserdebuggertools.targets_manager."
...
@pytest.fixture()
def ws_session_manager(producer):
...
class Test_ws_session_manager_reset(SessionManagerTest):
...
class Test_WSSessionManager__increment_message_producer_not_ok:
    ...
    @pytest.fixture()
    def _ws_session_manager(self, ws_session_manager):
        ws_session_manager.MAX_RETRY_THREADS = 3
        ws_session_manager.RETRY_COUNT_TIMEOUT = 300
        return ws_session_manager

    def test_first_run_on_ws(self, _ws_session_manager):
        _ws_session_manager._last_not_ok = None

        _ws_session_manager._increment_message_producer_not_ok()

        assert 100 == _ws_session_manager._last_not_ok
        assert 1 == _ws_session_manager._message_producer_not_ok_count

    def test_increment(self, _ws_session_manager):

        _ws_session_manager._last_not_ok = 49
        _ws_session_manager._message_producer_not_ok_count = 1

        _ws_session_manager._increment_message_producer_not_ok()

        assert 100 == _ws_session_manager._last_not_ok
        assert 2 == _ws_session_manager._message_producer_not_ok_count

    def test_timeout_expired(self, _ws_session_manager):
        _ws_session_manager.RETRY_COUNT_TIMEOUT = 50
        _ws_session_manager._last_not_ok = 49
        _ws_session_manager._message_producer_not_ok_count = 3
    ...
    def test_exceeded_max_failures(self, _ws_session_manager):
        _ws_session_manager._last_not_ok = 49
        _ws_session_manager._message_producer_not_ok_count = 3
        _ws_session_manager._exception = None

        with pytest.raises(MaxRetriesException):
            _ws_session_manager._increment_message_producer_not_ok()

        assert 100 == _ws_session_manager._last_not_ok
        assert 4 == _ws_session_manager._message_producer_not_ok_count
...
class Test_Target(unittest.TestCase):
    ...
    def test_id(self):
        self.assertEqual(4, self.target.id)

    def test_type(self):
        self.assertEqual("Page", self.target.type)

    @patch(MODULE_PATH + "_WSSessionManager")
    def test_attach(self, _WSSessionManager):
        self.target.attach()

        _WSSessionManager.assert_called_once_with(
            "ws://localhost:9222/devtools/page/1", 10, domains={"Page": {}}
        )
        self.assertEqual(
            _WSSessionManager.return_value,
            self.target.dom_manager._socket_handler
        )

    @patch(MODULE_PATH + "_WSSessionManager")
    def test_detach(self, _WSSessionManager):
        self.target.attach()

        self.target.detach()

        _WSSessionManager.return_value.close.assert_called_once_with()

    @patch(MODULE_PATH + "_WSSessionManager")
    def test_reset(self, _WSSessionManager):
        self.target.attach()
        self.target.dom_manager.reset = MagicMock()

        self.target.reset()

        _WSSessionManager.return_value.reset.assert_called_once_with()
        self.target.dom_manager.reset.assert_called_once_with()

    @patch(MODULE_PATH + "_WSSessionManager")
    def test_reset_notAttached(self, _WSSessionManager):
        with self.assertRaises(TargetNotAttachedError):
            self.target.reset()


class Test_TargetsManager_get_opened_javascript_dialog:

    def test(self, targets_manager):
        eventHandler = MagicMock()
        targets_manager.current_target.wsm.event_handlers.javascriptDialog = eventHandler

        dialog = targets_manager.get_opened_javascript_dialog()

        assert dialog == eventHandler.get_opened_javascript_dialog.return_value


class Test_TargetsManager_get_page_source:

    def test(self, targets_manager):
        eventHandler = MagicMock()
        targets_manager.current_target.wsm.event_handlers.pageLoad = eventHandler

        source = targets_manager.get_page_source()

        assert source == targets_manager.current_target.dom_manager.get_outer_html.return_value
        targets_manager.current_target.dom_manager.get_outer_html.assert_called_once_with(
            eventHandler.get_root_backend_node_id.return_value
        )


class Test_TargetsManager_get_iframe_source_content:

    def test(self, targets_manager):
        content = targets_manager.get_iframe_source_content("x-path")

        assert targets_manager.current_target.dom_manager.get_iframe_html.return_value == content
        targets_manager.current_target.dom_manager.get_iframe_html.assert_called_once_with("x-path")


class Test_TargetsManager_get_url:

    def test(self, targets_manager):
        eventHandler = MagicMock()
        targets_manager.current_target.wsm.event_handlers.pageLoad = eventHandler

        url = targets_manager.get_url()

        assert url == eventHandler.get_current_url()
...
class Test_TargetsManager_get_events:

    def test(self, targets_manager):
        events = targets_manager.get_events("Page", clear=True)

        assert targets_manager.current_target.wsm.get_events.return_value == events
        targets_manager.current_target.wsm.get_events.assert_called_once_with("Page", clear=True)


class Test_TargetsManager_execute:

    def test(self, targets_manager):
        result = targets_manager.execute("domain", "method", params={"foo": "baa"})

        assert targets_manager.current_target.wsm.execute.return_value == result
...
class Test_TargetsManager_enable_domain:
    ...
    def test(self, targets_manager):
        ...
        targets_manager._targets[
            targets_manager.current_target_id
        ].wsm.enable_domain.assert_called_once_with(
            "Fetch", parameters={"foo": "bar"}
        )


class Test_TargetsManager_disable_domain:

    def test(self, targets_manager):
        targets_manager.enable_domain("Fetch", parameters={"foo": "bar"})

        targets_manager.disable_domain("Fetch")

        targets_manager.current_target.wsm.disable_domain("Fetch")
        assert "Fetch" not in targets_manager._domains

    def test_domainNotEnabled(self, targets_manager):
        try:
            targets_manager.disable_domain("Fetch")
        except DomainNotEnabledError:
            pass
        else:
            raise Exception("Expected DomainNotEnabledError")


class Test_TargetsManager_switch_target_and_current_target:

    def test(self, targets_manager):
        targets_manager.switch_target("1")

        assert targets_manager.current_target.id == "1"

        targets_manager.switch_target("3")

        assert targets_manager.current_target.id == "3"
...
class Test_TargetsManager_detach_all:
    ...
    def test(self, targets_manager):
        ...
        webSocketManagers = [target.wsm for target in targets]
        targets_manager.detach_all()

        for webSocketManager in webSocketManagers:
            webSocketManager.close.assert_called_once_with()
...
class Test_TargetsManager_refresh_targets:
    ...
    def test(self, targets_manager):
        ...
        self._check(actual=targets_manager.targets, expected={
            "1": {
                "id": "1", "type": "page",
                "webSocketDebuggerUrl": "ws://localhost:9222/devtools/page/1"
            },
            ...
            "5": {
                "id": "5", "type": "page",
                "webSocketDebuggerUrl": "ws://localhost:9222/devtools/page/5"
            }
        })

        targets_manager.refresh_targets()
        self._check(
            actual=targets_manager.targets,
            expected={
                # "6": {
                #     "id": "6", "type": "page",
                #     "webSocketDebuggerUrl": "ws://localhost:9222/devtools/page/6"
                # },
                "7": {
                    "id": "7", "type": "page",
...
class Test_TargetsManager_targets:

    def test(self, targets_manager):
        targets = targets_manager.targets

        assert [targetID for targetID in targets.keys()] == ["1", "2", "3", "4", "5"]
...
class Test_TargetsManager__create_tab:
...
class DOMManagerTest(TestCase):
    ...
    def setUp(self):
        self.socket_handler = MagicMock()
        self.dom_manager = _DOMManager(self.socket_handler)


class Test__DOMManager_get_outer_html(DOMManagerTest):

    def test(self):
        self.socket_handler.execute.return_value = {"outerHTML": "<html></html>"}

        outerHTML = self.dom_manager.get_outer_html(5)

        self.assertEqual(outerHTML, "<html></html>")
        self.socket_handler.execute.assert_called_once_with(
            "DOM", "getOuterHTML", {"backendNodeId": 5}
        )


class Test__DOMManager_get_iframe_html(DOMManagerTest):
...
class Test__DOMManager__get_iframe_backend_node_id(DOMManagerTest):
    ...
    def test_cached_but_reset(self):
        self.dom_manager._node_map = {"//iframe": 5}
        node_info = {
            "node": {
                "contentDocument": {
...
class Test__DOMManager__get_info_for_first_matching_node(DOMManagerTest):
...
class Test__DOMManager_perform_search(DOMManagerTest):

    def test(self):
        self.dom_manager._perform_search("x-path")

        self.socket_handler.execute.assert_called_once_with(
            "DOM", "performSearch", {"query": "x-path"}
        )
        self.socket_handler.event_handlers.pageLoad.check_page_load.assert_called_once_with()


class Test__DOMManager_get_search_results(DOMManagerTest):

    def test(self):
        self.dom_manager._get_search_results("search-id", 3, 8)

        self.socket_handler.execute.assert_called_once_with("DOM", "getSearchResults", {
            "searchId": "search-id", "fromIndex": 3, "toIndex": 8
        })


class Test__DOMManager_discard_search(DOMManagerTest):

    def test(self):
        self.dom_manager._discard_search("search-id")

        self.socket_handler.execute.assert_called_once_with(
            "DOM", "discardSearchResults", {"searchId": "search-id"}
        )


class Test__DOMManager_describe_node(DOMManagerTest):

    def test(self):
        self.dom_manager._describe_node("node-id")

        self.socket_handler.execute.assert_called_once_with(
            "DOM", "describeNode", {"nodeId": "node-id"}
        )
